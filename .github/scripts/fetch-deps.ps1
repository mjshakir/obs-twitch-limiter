...
if (Test-Path $libobsPath) {
    ...
    # Configure CMake with flags to install libobs’s CMake config.
    # The flag -DCMAKE_INSTALL_LIBDIR="lib/cmake/libobs" is used to direct the installation
    # of configuration files to the expected folder.
    ...
    # Define the fallback libobs config directory (where libobsConfig.cmake should be).
    ...
    # Define the expected config file.
    $fallbackConfigFile = Join-Path $fallbackLibobsPath "libobsConfig.cmake"

    # If the config file doesn't exist, generate a minimal one.
