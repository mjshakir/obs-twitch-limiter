...
bool obs_module_load(void) {
    ...
    EventSub::set_status_callback(update_websocket_status);
    EventSub::set_overlay_callback(show_overlay_notification);
    EventSub::initialize();
    return true;
}
...
obs_properties_t* obs_module_get_settings(void* data) {
    static_cast<void>(data);
    std::unique_ptr<obs_properties_t, decltype(&obs_properties_destroy)> props(obs_properties_create(), &obs_properties_destroy);

    obs_property_t* limit_toggle = obs_properties_add_bool(props.get(), "enable_custom_bet_limit", "Enable Custom Bet Limit", custom_bet_limit_enabled);
    ...
    static_cast<void>(obs_properties_add_int(props.get(), "bet_timeout_duration", "Bet Timeout Duration (seconds)", 5, 300, 5));

    static_cast<void>(obs_properties_add_button(props.get(), "reset_bet_limit", "Reset Bet Limit", reset_bet_limit));
    static_cast<void>(obs_properties_add_button(props.get(), "reset_bet_timeout", "Reset Bet Timeout", reset_bet_timeout));
    static_cast<void>(obs_properties_add_button(props.get(), "reset_overlay", "Reset Overlay", reset_overlay));

    obs_property_t* ws_url_prop = obs_properties_add_text(props.get(), "websocket_url", "WebSocket URL", OBS_TEXT_DEFAULT);
    obs_property_set_modified_callback(ws_url_prop, validate_websocket_url);
    
    static_cast<void>(obs_properties_add_button(props.get(), "reset_websocket_url", "Reset WebSocket URL", reset_websocket_url));

    static_cast<void>(obs_properties_add_button(props.get(), "manual_reconnect_eventsub", "Reconnect to Twitch EventSub", manual_reconnect_eventsub));

    obs_property_t* ws_status = obs_properties_add_text(props.get(), "ws_status", "WebSocket Status", OBS_TEXT_INFO);
...
bool validate_websocket_url(obs_properties_t* props, obs_property_t* prop, void* data) {
    ...
    if (new_url && *new_url) {
        if (is_valid_websocket_url(new_url)) {
...
void obs_module_update_settings(obs_data_t* settings) {
    ...
    EventSub::set_max_bet_limit(custom_bet_limit_enabled ? max_bet_limit : SIZE_MAX);
    EventSub::set_bet_timeout_duration(bet_timeout_duration);

    obs_log_info("Updated WebSocket URL: %s", websocket_url.c_str());
    obs_log_info("Updated Bet Limit: %s", custom_bet_limit_enabled ? std::to_string(max_bet_limit).c_str() : "Disabled");
...
void update_websocket_status(bool is_connected) {
    ...
    obs_data_set_string(settings, "ws_status", is_connected ? "Connected ✅" : "Disconnected ❌");
    obs_frontend_push_ui_translation(is_connected ? "Connected to Twitch EventSub!" : "WebSocket Disconnected!");
}